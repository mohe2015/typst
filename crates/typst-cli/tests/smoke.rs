...
#[test]
fn test_tracepoints_numbering2() {
    let project = tempfs();
    let main = project.write(
        "main.typ",
        r#"#set heading(numbering: (n, trimmed: true) => if trimmed { str(n) } else { assert(false) })
           = Heading"#,
    );
    let output = exec().arg("compile").arg(&main).must_fail();
    println!("{}", output.stderr);
...
#[test]
fn test_tracepoints_numbering3() {
    let project = tempfs();
    let main = project.write(
        "main.typ",
        r#"#numbering((n, trimmed: true) => if trimmed { str(n) } else { assert(false) }, 1)"#,
    );
    let output = exec().arg("compile").arg(&main).must_fail();
    println!("{}", output.stderr);
    output
